...
if (isset($_POST['grupo_vulnerable'])) {
    $idUsuario = $_POST['id_usuario']; // Recuperar la variable id_usuario
    $grupoVulnerable = $_POST['grupo_vulnerable'];

    // Consulta para obtener el tutor_id
    $consulta_tutor = "
    ...
    $tutor_result = mysqli_query($conexion, $consulta_tutor);
    ...
    if (!$tutor_data) {
        die("No se encontró un tutor asociado al usuario con ID: $idUsuario");
    }

    $tutor_id = $tutor_data['tutor_id']; // Obtener el tutor_id

    // Depuración: Verificar el valor de tutor_id
    // var_dump($tutor_id);
    // exit;

    // Continuar con las demás consultas según $grupoVulnerable...
