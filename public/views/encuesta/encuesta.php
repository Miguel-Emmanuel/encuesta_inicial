...
while ($preguntas = $sql->fetch_object()) {
    ...
    $tipoPregunta = $preguntas->tipo; // Supongamos que el tipo de pregunta está almacenado en una columna llamada 'tipo'

    echo "<div class='pregunta'>";
    echo "<p class='pregunta-texto'>$idPregunta.   <b>$preguntaTexto</b></p>";

    // Generar diferentes tipos de entrada según el tipo de pregunta
    switch ($tipoPregunta) {
        case 'texto':
            echo "<input type='text' name='respuestas[$idPregunta]' class='respuesta-input' placeholder='Respuesta para la pregunta'>";
            break;
            case 'opcion':
                // Aquí integras el código para opciones_respuesta
                // Inicia el código para opciones_respuesta
                ...
                if ($opciones_respuesta->num_rows > 0) {
                    ...
                    // Recorrer todas las filas de opciones para esta pregunta
                    echo "<table>";
                ...
                // Termina el código para opciones_respuesta
                break;
                case 'select':
                    // Código para mostrar opciones con un select
                    $opciones_respuesta = $conexion->query("SELECT * FROM opciones_respuesta WHERE pregunta_id = $idPregunta");
                    // Verificar si hay resultados
                    if ($opciones_respuesta->num_rows > 0) {
                    ...
                    case 'multi':
                        // Verificar si hay datos en opción1 y opción2
                        $opciones_respuesta = $conexion->query("SELECT * FROM opciones_respuesta WHERE pregunta_id = $idPregunta");
                        if ($opciones_respuesta->num_rows > 0) {
                            echo "<div class='pregunta'>";
                            echo "<p class='pregunta-texto'>Opciones para pregunta ID: $idPregunta</p>";
                            
                            // Crear un array para almacenar las opciones de opc1 y opc2
                            $opciones = array();
                            
                            // Recorrer todas las filas de opciones para esta pregunta y almacenarlas en el array
                            while ($opcion = $opciones_respuesta->fetch_object()) {
                                $opciones[$opcion->opcion1][] = $opcion->opcion2;
                            }
                            
                            // Verificar si hay valores en el array de opciones
                            if (!empty($opciones)) {
                                echo "<table>";
                                echo "<tr><th>Opción 1</th>"; // Cabecera para opc1
                                // Imprimir una fila para cada valor de opc2
                                foreach ($opciones[array_key_first($opciones)] as $opcion2) {
                                    echo "<th>$opcion2</th>";
                                }
                                echo "</tr>";
                                
                                // Recorrer el array de opciones y construir la cuadrícula
                                foreach ($opciones as $opcion1 => $valoresOpcion2) {
                                    echo "<tr>";
                                    echo "<td>$opcion1</td>"; // Imprimir el valor de opc1
                                    
                                    // Recorrer los valores de opc2 y agregar checkboxes
